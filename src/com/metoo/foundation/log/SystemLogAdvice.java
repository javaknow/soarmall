package com.metoo.foundation.log;
...
import com.metoo.foundation.domain.LogFieldType;
...
public class SystemLogAdvice {
	...
	// 记录管理员操作日志[通知（Advice）切面（Aspect），切点（PointCut）-连接点（JoinPoint）]
	@After(value = "execution(* com.metoo.manage.admin..*.*(..)) && @annotation(annotation)&&args(request,..)")
	...
	//记录用户保存用户信息
	@After(value = "execution(* com.metoo.manage.buyer..*.*(..)) && @annotation(annotation)&&args(request,..)")
	public void buyer_account(JoinPoint joinPoint,
	...
	@After(value = "execution(* com.metoo.module.weixin.manage..*.*(..)) && @annotation(annotation)&&args(request,..)")
	...
	@After(value = "execution(* com.metoo.module.app.manage..*.*(..)) && @annotation(annotation)&&args(request,..)")
	...
	@After(value = "execution(* com.metoo.module.cms.manage..*.*(..)) && @annotation(annotation)&&args(request,..)")
	...
	@After(value = "execution(* com.metoo.module.circle.manage..*.*(..)) && @annotation(annotation)&&args(request,..)")
	...
	@After(value = "execution(* com.metoo.module.sns.manage..*.*(..)) && @annotation(annotation)&&args(request,..)")
	...
	// 异常日志记录
	@AfterThrowing(value = "execution(* com.metoo.manage.admin..*.*(..))&&args(request,..) ", throwing = "exception")
