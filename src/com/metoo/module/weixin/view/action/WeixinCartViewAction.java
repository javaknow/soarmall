...
public class WeixinCartViewAction {
	 ...
	 * @return
	 */
	 @SecurityMapping(title = "查看购物车", value = "/wap/goods_cart1.htm*", rtype = "buyer", rname = "购物流程0", rcode = "wap_goods_cart", rgroup =
	 "移动端购物")
	...
	public ModelAndView goods_cart1(HttpServletRequest request,
		...
		if (carts.size() > 0) {
			...
			for (GoodsCart cart : carts) {
				//[满就送状态，0为非满就送商品，1为满就送商品] [对应的满就送id]
				if (cart.getGoods().getOrder_enough_give_status() == 1
					...
					BuyGift bg = this.buyGiftService.getObjById(cart.getGoods()
							.getBuyGift_id());
					if (bg.getBeginTime().before(date)) {//[bg.getBeginTime 小与 data是 true 活动结束] 
						set.add(cart.getGoods().getBuyGift_id());
						//[否则添加购物车]
					} else {
						native_goods.add(cart);
					}
				} else if (cart.getGoods().getEnough_reduce() == 1) {// 满就减  [ 0为未参加满就减，1为已参加]
					//[根据对应的满就减id查询开始时间]
