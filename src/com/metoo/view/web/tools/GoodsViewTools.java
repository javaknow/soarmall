...
public class GoodsViewTools {
	...
	public List<Goods> self_goods_sales(int count) {
		List<Goods> list = new ArrayList<Goods>();
		Map params = new HashMap();
		//params.put("goods_type", goods_type);
		params.put("goods_status", 0);
		//["select new Goods(id, goods_name, goods_current_price,goods_collect, goods_salenum,goods_main_photo,goods_price) from Goods obj where obj.goods_type=:goods_type and obj.goods_status=:goods_status order by obj.goods_salenum desc",]
		list = this.goodsService
				.query("select obj from Goods obj where obj.goods_status=:goods_status order by obj.goods_salenum desc",
						params, 0, count);
		return list;
	}

	
	public List<Goods> self_goods_sales_temp(int count) {
		List<Goods> list = new ArrayList<Goods>();
		Map params = new HashMap();
		params.put("goods_status", 0);
		list = this.goodsService
				.query("select obj.id, obj.goods_name, obj.goods_current_price,obj.goods_collect, obj.goods_salenum,sobj.path,sobj.name,sobj.ext from Goods as obj left join Accessory as sobj on obj.goods_main_photo_id=sobj.id where obj.goods_status=:goods_status order by goods_salenum desc",
						params, 0, count);
		return list;
	}
	 ...
	 * @return
	 */
	public List<Goods> queryGoodsdetails(int count){
		Map params = new HashMap();
		params.put("goods_status", 0);
		//params.put("goods_type", goods_type);
	...
	public List queryGoodsdetail(){
		Map goodsmap = new HashMap();
		goodsmap.put("goods_status", 0);
		List<Goods> goodslist = this.goodsService
				.query("select obj from Goods obj where obj.goods_status=:goods_status order by obj.goods_name desc", goodsmap,
		-1, -1);
