...
public class LoginViewAction {
	...
	public String user_login_success(HttpServletRequest request,
		...
		//[用于目前只开放商家后台的操作，用户注册完成后直接跳转登录入口，认证成功进入该方法，如何登录成功则进入商家入驻页面 seller_apply.htm]
	...
	/**
	 * code 0 ：成功 1：失败 2 ：用户没有绑定手机号 3:没有改用户
	 * app修改密码
	 */
	...
	public void app_forget(HttpServletRequest request,
		...
		SysConfig config = this.configService.getSysConfig();
		if (!config.isEmailEnable() && !config.isSmsEnbale()) {
			result = new Result(1,"系统关闭邮件及手机短信功能，不能找回密码");
		} else {
			//根据手机号获取验证码
			...
			if (mvc != null && mvc.getCode().equalsIgnoreCase(mobile_verify_code)) {
					...
					// 绑定成功后发送手机短信提醒
					String content = "尊敬的"
							+ "Ebuyair用户您好，您于" + CommUtil.formatLongDate(new Date())
							+ "验证身份成功。["
					...
					if (users.size() > 0) {
						...
						if (!CommUtil.null2String(user.getMobile()).equals("")) {
							...
							result = new Result(2,"用户没有绑定手机号");
						}

					} else {
						result = new Result(3, "请先注册用户,或使用本人手机号" );
					}
	...
	public ModelAndView forget4(HttpServletRequest request,
		...
		if (!verify_session1.equals("")
			...
			mv.addObject("url", CommUtil.getURL(request) + "/user_login.json");
		} else {
			mv = new JModelAndView("error.html", configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 1, request,
					response);
