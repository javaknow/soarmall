...
import com.metoo.core.tools.XMLUtil;
...
import com.metoo.msg.SpelTemplate;
...
public class CartViewAction {
	...
	private List<GoodsCart> cart_calc(HttpServletRequest request) {
		...
		if (user != null) {
			...
			carts_user = this.goodsCartService.query(
					"select obj from GoodsCart obj where obj.user.id=:user_id and obj.cart_status=:cart_status order by obj.addTime desc asc",
					cart_map, -1, -1);
	...
	/*
	 * @RequestMapping("/cart_menu_detail.htm") public ModelAndView
	 * cart_menu_detail(HttpServletRequest request, HttpServletResponse
	 * response) { ModelAndView mv = new JModelAndView("cart_menu_detail.html",
	 * configService.getSysConfig(), this.userConfigService.getUserConfig(), 1,
	 * request, response); List<GoodsCart> carts = this.cart_calc(request); if
	 * (carts.size() > 0) { mv.addObject("total_price", this.calCartPrice(carts,
	 * "")); mv.addObject("carts", carts); } return mv; }
	 */
	...
	public void add_goods_cart(HttpServletRequest request, HttpServletResponse response, String id, String count,
			String price, String gsp, String buy_type, String combin_ids, String combin_version) {
		Result result = null;
		...
		if (goods.getGoods_status() == 0) {
			...
			int goods_inventory = CommUtil.null2Int(this.generic_default_info(goods, gsp).get("count"));// 计算商品库存信息
					...
					for (String part_id : part_ids) {
						if (!part_id.equals("")) {
							...
							if (part_add) {// 排除购物车中没有重复商品后添加该商品到购物车
								...
								String part_price = this.generGspgoodsPrice(temp_gsp_parts, part_id);
						...
						if (gc != null) {
							if (gc.getCart_type() != null && gc.getCart_type().equals("combin")) {// 如果商城的购物车为组合套装，则删除最后一个组合套装购物车时，组合套装主购物车更随删除
								...
								if (suit_main_carts.size() > 0) {
									String suit_cart_ids[] = suit_main_carts.get(0).getCombin_suit_ids().split(",");
									// 取消其他购物车组合状态
									for (String suit_cart_id : suit_cart_ids) {
										if (!suit_cart_id.equals("")) {
											...
											if (suit_cart != null) {
												...
												// 设置默认规格及价钱2Long(id));
												String default_gsp = this.generic_default_gsp(suit_cart.getGoods());
												double default_price = CommUtil.null2Double(
														this.generic_default_info(suit_cart.getGoods(), default_gsp)
																.get("price"));
									...
									for (GoodsCart main_suit_gc : suit_main_carts) {
										...
										double default_price = CommUtil.null2Double(
												this.generic_default_info(main_suit_gc.getGoods(), default_gsp)
														.get("price"));
	...
	private Map calEnoughReducePrice(List<GoodsCart> carts, String gcs) {
		...
		for (String er_id : ermap.keySet()) {
			...
			for (Object enough : fromJson.keySet()) {
				if (er_money >= CommUtil.null2Double(enough)) {
					reduce = CommUtil.null2Double(fromJson.get(enough));
					erstr = "The activity products already bought full" + enough + " AED.  Has been reduced " + reduce
							+ " AED";
	...
	private String generGspgoodsPrice(String gsp, String id) {
		...
		if ("spec".equals(goods.getInventory_type())) {
			...
			if (user != null && goods.getActivity_status() == 2) {
				Map map = this.activityTools.getActivityGoodsInfo(CommUtil.null2String(goods.getId()),
						CommUtil.null2String(user.getId()));
				Double rate = CommUtil.div(CommUtil.null2Double(map.get("rate")), 10);
				price = CommUtil.null2Double(CommUtil.formatMoney(CommUtil.mul(rate, price)));
			}
		}
		...
		if (gc != null) {
			if (CommUtil.null2String(count).length() <= 9) {
				if (gc.getId().toString().equals(gc_id)) {
					...
					// 判断出是否满足满就送条件
					if (gift_id != null) {
						BuyGift bg = this.buyGiftService.getObjById(CommUtil.null2Long(gift_id));
	...
	public void combin_carts_detail(HttpServletRequest request, HttpServletResponse response, String id) {
		...
		if (cart != null) {
			if (cart.getCart_type() != null && cart.getCart_type().equals("combin") && cart.getCombin_main() == 1) {
					...
					if (!cart_id.equals("") && !cart_id.equals(id)) {
						...
						if (other != null) {
							...
							if (other.getGoods().getGoods_main_photo() != null) {
								img2 = CommUtil.getURL(request) + "/" + other.getGoods().getGoods_main_photo().getPath()
		...
		if (type != null && !type.equals("")) {
			if (type.equals("suit")) {
				...
				mv = new JModelAndView("error.html", configService.getSysConfig(),
						this.userConfigService.getUserConfig(), 1, request, response);
				mv.addObject("op_title", "The address you are visiting does not exist");
	...
	public ModelAndView goods_cart0(HttpServletRequest request, HttpServletResponse response, String gid) {
		...
		if (goods != null) {
			...
			List<Goods> class_goods = this.goodsService.query(
					"select obj from Goods obj where obj.gc.id=:gc_id and obj.id!=:gid and obj.goods_status=:goods_status order by goods_salenum desc",
					map, 0, 9);
	...
	public void goods_cart1_spec_save(HttpServletRequest request, HttpServletResponse response, String gsp, String id) {
		...
		if (obj != null) {
			int goods_inventory = CommUtil.null2Int(this.generic_default_info(obj.getGoods(), gsp).get("count"));// 计算商品库存信息
			double price = CommUtil.null2Double(this.generic_default_info(obj.getGoods(), gsp).get("price"));// 计算商品库存信息
			if (goods_inventory == 0) {
				code = -100;
			} else {
	...
	public ModelAndView goods_cart2(HttpServletRequest request, HttpServletResponse response, String gcs,
		...
		if (gcs == null || gcs.equals("")) {
			mv = new JModelAndView("error.html", configService.getSysConfig(), this.userConfigService.getUserConfig(),
					1, request, response);
			mv.addObject("op_title", "The parameter is wrong, please re-enter the shopping cart");
			mv.addObject("url", CommUtil.getURL(request) + "/index.htm");
			return mv;
		}
		...
		if (carts.size() > 0) {
			for (GoodsCart gc : carts) {
				if (!gc.getUser().getId().equals(SecurityUserHolder.getCurrentUser().getId())) {
					flag = false;
					break;
				}
			}
		...
		if (flag && carts.size() > 0) {
			...
			for (int i = 0; i < 7; i++) {
				...
				day_list.add(
						CommUtil.formatTime("MM-dd", cal.getTime()) + this.generic_day(cal.get(Calendar.DAY_OF_WEEK)));
			}
			// 比较当日时间段
			Calendar cal = Calendar.getInstance();
			...
			mv = new JModelAndView("error.html", configService.getSysConfig(), this.userConfigService.getUserConfig(),
					1, request, response);
			mv.addObject("op_title", "Cart information is empty");
			mv.addObject("url", CommUtil.getURL(request) + "/index.htm");
		}
			...
			for (String gc_id : gc_ids) {
				...
				if (gc != null && gc.getGoods().getGoods_cod() == -1) {// 只要存在一件不允许使用货到付款购买的商品整个订单就不允许使用货到付款
					if (!payType.equals("online")) {// 订单不支持货到付款，用户从页面前端恶意篡改支付方式为货到付款
						mv = new JModelAndView("error.html", configService.getSysConfig(),
								this.userConfigService.getUserConfig(), 1, request, response);
						mv.addObject("op_title", "You maliciously modify the payment method, the order has expired");
						mv.addObject("url", CommUtil.getURL(request) + "/index.htm");
						return mv;
					}
				}
			...
			if (order_carts.size() > 0 && addr != null) {
				...
				for (GoodsCart gc : order_carts) {
					...
					int goods_inventory = CommUtil
							.null2Int(this.generic_default_info(gc.getGoods(), gc.getCart_gsp()).get("count"));// 计算商品库存信息
				...
				if (inventory_very) {
					User buyer = this.userService
							.getObjById(CommUtil.null2Long(SecurityUserHolder.getCurrentUser().getId()));
					...
					for (int i = 0; i < store_ids.length; i++) {// 根据店铺id，保存多个子订单
							...
							if (gc.getGoods().getGoods_type() == 1) {// 商家商品
								...
								if (add) {
									if (sid == "self" || sid.equals("self")) {
										...
										final String genId = SecurityUserHolder.getCurrentUser().getId()
						...
						String er_json = (String) ermap.get("er_json");//[订单中参加的满就减的json]
						double all_goods = Double.parseDouble(ermap.get("all").toString());//[商品总价格]
						double reduce = Double.parseDouble(ermap.get("reduce").toString());//[满就减的金额]
						...
						of.setOrder_id(SecurityUserHolder.getCurrentUser().getId() + order_suffix + order_store_id);
						// 设置收货地址信息
					...
					mv.addObject("all_of_price", CommUtil.formatMoney(all_of_price));
					mv.addObject("paymentTools", paymentTools);

				} else {// 验证库存不成功，返回购物车，并给出提示！
					mv = new JModelAndView("error.html", configService.getSysConfig(),
							this.userConfigService.getUserConfig(), 1, request, response);
					mv.addObject("op_title", "The goods are out of stock. Please delete it");
	...
	public ModelAndView order_pay_view(HttpServletRequest request, HttpServletResponse response, String id) {
		...
		if (of != null && of.getUser_id().equals(user.getId().toString())) {
			if (of.getOrder_status() == 10) {
				if (of.getOrder_cat() == 1) {// 处理手机充值付款
					...
					if (CommUtil.null2Double(map.get("inprice")) <= CommUtil.null2Double(rc_amount)) {
						inprice = CommUtil.add(map.get("inprice"),
								this.configService.getSysConfig().getOfcard_mobile_profit());
					...
					for (Goods obj : goods_list) {
						...
						List<Map> goods_maps = this.orderFormTools.queryGoodsInfo(of.getGoods_info());
						...
						// 真实商品库存
						int real_goods_count = CommUtil
								.null2Int(this.generic_default_info(obj, order_goods_gsp_ids).get("count"));// 计算商品库存信息
					...
					if (inventory_very) {// 订单中商品库存验证成功
						...
						mv = new JModelAndView("error.html", configService.getSysConfig(),
								this.userConfigService.getUserConfig(), 1, request, response);
						mv.addObject("op_title", "The goods are out of stock. Please place an order again");
				...
				mv = new JModelAndView("error.html", configService.getSysConfig(),
						this.userConfigService.getUserConfig(), 1, request, response);
				mv.addObject("op_title", "The order has been cancelled");
				mv.addObject("url", CommUtil.getURL(request) + "/index.htm");
			} else {
				mv = new JModelAndView("error.html", configService.getSysConfig(),
						this.userConfigService.getUserConfig(), 1, request, response);
				mv.addObject("op_title", "This order has been paid");
				mv.addObject("url", CommUtil.getURL(request) + "/buyer/order.htm");
			}
		} else {
			mv = new JModelAndView("error.html", configService.getSysConfig(), this.userConfigService.getUserConfig(),
					1, request, response);
			mv.addObject("op_title", "The order has expired");
			mv.addObject("url", CommUtil.getURL(request) + "/buyer/order.htm");
		}
		return mv;
	}
		...
		if (order != null && order.getOrder_status() == 10) {
			...
			if (inventory_very) {// 订单中商品库存验证成功
				if (CommUtil.null2String(payType).equals("")) {
					...
					payments = this.paymentService.query("select obj from Payment obj where obj.mark=:mark", params, -1,
							-1);
			...
			mv = new JModelAndView("error.html", configService.getSysConfig(), this.userConfigService.getUserConfig(),
					1, request, response);
			mv.addObject("op_title", "Parameter error, payment failure");
			mv.addObject("url", CommUtil.getURL(request) + "/index.htm");
		}
		return mv;
	}
	...
	public ModelAndView order_pay_payafter(HttpServletRequest request, HttpServletResponse response, String payType,
		...
		if (pay_session1.equals(pay_session)) {
			...
			mv.addObject("op_title", "The COD was successful，please wait.");
			mv.addObject("url", CommUtil.getURL(request) + "/buyer/order.htm");
		} else {
			mv = new JModelAndView("error.html", configService.getSysConfig(), this.userConfigService.getUserConfig(),
					1, request, response);
			mv.addObject("op_title", "This order has been paid，please do not submit again");
			mv.addObject("url", CommUtil.getURL(request) + "/buyer/order.htm");
		}
		return mv;
	}
			...
			if (CommUtil.null2Double(user.getAvailableBalance()) >= order_total_price) {
				...
				if (ret) {
					...
					if (order.getOrder_cat() == 2) {
						...
						if (order.getOrder_form() == 0) { //[订单种类，0为商家商品订单，1为平台自营商品订单]
							...
							store.setStore_sale_amount(BigDecimal
									.valueOf(CommUtil.add(order.getTotalPrice(), store.getStore_sale_amount())));// 店铺本次结算总销售金额
							// 团购消费码，没有佣金，店铺总佣金不变
							store.setStore_payoff_amount(BigDecimal
									.valueOf(CommUtil.add(order.getTotalPrice(), store.getStore_payoff_amount())));// 店铺本次结算总佣金
							this.storeService.update(store);
						}
						// 增加系统总销售金额、消费码没有佣金，系统总佣金不变
						SysConfig sc = this.configService.getSysConfig();
						sc.setPayoff_all_sale(
								BigDecimal.valueOf(CommUtil.add(order.getTotalPrice(), sc.getPayoff_all_sale())));
					...
					user.setAvailableBalance(
							BigDecimal.valueOf(CommUtil.subtract(user.getAvailableBalance(), order_total_price)));
									...
									for (Goods temp : goods_list) {
										if (!CommUtil.null2String(temp_goods.get("id"))
											...
											Goods goods = this.goodsService
													.getObjById(CommUtil.null2Long(temp_goods.get("id")));
				...
				mv = new JModelAndView("error.html", configService.getSysConfig(),
						this.userConfigService.getUserConfig(), 1, request, response);
				mv.addObject("op_title", " Insufficient amount, payment failed");
				mv.addObject("url", CommUtil.getURL(request) + "/buyer/order.htm");
			}
		} else {
			mv = new JModelAndView("error.html", configService.getSysConfig(), this.userConfigService.getUserConfig(),
					1, request, response);
			mv.addObject("op_title", "Parameter error, payment failed");
			mv.addObject("url", CommUtil.getURL(request) + "/buyer/order.htm");
		}
		return mv;
	}
	...
	public ModelAndView cart_address_save(HttpServletRequest request, HttpServletResponse response, String id,
			String area_id, String op_type, String gcs) {
	...
	public void order_address(HttpServletRequest request, HttpServletResponse response, String addr_id,
		...
		for (GoodsCart gc : carts) {
			if (store_id != "self" && !store_id.equals("self")) {
				if (gc.getGoods().getGoods_type() == 1
						&& gc.getGoods().getGoods_store().getId().equals(CommUtil.null2Long(store_id))) {
					gc_list.add(gc);
				}
			} else {
	...
	public ModelAndView f_code_cart(HttpServletRequest request, HttpServletResponse response, String goods_id,
			String gsp) {
			...
			if (goods.getF_sale_type() == 0) {
				...
				if (goods_inventory > 0) {
					...
					if (this.goodsViewTools.generic_spec(goods_id).size() == specs.size()) {// 这里判断传入的规格数量和商品本身具有的是否一致,杜绝用户私自修改传递参数
						...
						mv = new JModelAndView("error.html", configService.getSysConfig(),
								this.userConfigService.getUserConfig(), 1, request, response);
						mv.addObject("op_title", "The size parameter error");
						mv.addObject("url", CommUtil.getURL(request) + "/index.htm");
					}
				} else {
					mv = new JModelAndView("error.html", configService.getSysConfig(),
							this.userConfigService.getUserConfig(), 1, request, response);
					mv.addObject("op_title", "The default size is not in stock, please choose other sizes to buy");
	...
	public void f_code_validate(HttpServletRequest request, HttpServletResponse response, String f_code,
			String goods_id) {
