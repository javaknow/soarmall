...
public class SellerApplyAction {
	...
	public ModelAndView seller_apply(HttpServletRequest request, HttpServletResponse response, String op, String step) {
		...
		if (user.getStore_apply_step() > 7) {// 申请分为7步，走完第7步流程进入审核状态，就不允许继续修改资料了
			...
			if (step != null && !CommUtil.null2String(step).equals("")) {// 如果step参数不为空，更新申请步骤
				if (user.getStore_apply_step() >= 0 && (CommUtil.null2Int(step) - user.getStore_apply_step() <= 1)) {// 如果用户已经进入申请流程，必须按照逐步流程走，杜绝恶意数据参数发生
	 ...
	 * @return
	 */
	private ModelAndView seller_apply_step0(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("seller_apply/seller_apply_step0.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		mv.addObject("doc", this.documentService.getObjByProperty(null, "mark", "apply_agreement"));
		return mv;
	}
	...
	private ModelAndView seller_apply_step1(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("seller_apply/seller_apply_step1.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		mv.addObject("doc", this.documentService.getObjByProperty(null, "mark", "apply_notice"));
		return mv;
	}
	 ...
	 * @return
	 */
	private ModelAndView seller_apply_step2(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("seller_apply/seller_apply_step2.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
	...
	public String seller_apply_step2_save(HttpServletRequest request, HttpServletResponse response, String trueName,
			String telephone, String email) {
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		if (!CommUtil.null2String(trueName).equals("") && !CommUtil.null2String(telephone).equals("")
	 ...
	 * @return
	 */
	private ModelAndView seller_apply_step3(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("seller_apply/seller_apply_step3.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
	...
	public String seller_apply_step3_save(HttpServletRequest request, HttpServletResponse response, String lid2,
			String cid2, String step) {
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user.setStore_apply_step(CommUtil.null2Int(step));
		this.userService.update(user);
		...
		store.setLicense_area(this.areaService.getObjById(CommUtil.null2Long(lid2)));
		store.setLicense_c_area(this.areaService.getObjById(CommUtil.null2Long(cid2)));
		this.storeService.update(store);
	 ...
	 * @return
	 */
	private ModelAndView seller_apply_step4(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("seller_apply/seller_apply_step4.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		List<Area> areas = this.areaService.query("select obj from Area obj where obj.parent.id is null", null, -1, -1);
		mv.addObject("areas", areas);
	...
	public String seller_apply_step4_save(HttpServletRequest request, HttpServletResponse response, String bid2,
			String step) {
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user.setStore_apply_step(CommUtil.null2Int(step));
		this.userService.update(user);
	 ...
	 * @return
	 */
	private ModelAndView seller_apply_step5(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("seller_apply/seller_apply_step5.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		...
		List<Area> areas = this.areaService.query("select obj from Area obj where obj.parent.id is null", null, -1, -1);
		List<StoreGrade> storeGrades = this.storegradeService.query("select obj from StoreGrade obj", null, -1, -1);
	...
	public String seller_apply_step5_save(HttpServletRequest request, HttpServletResponse response, String aid2,
			String step, String storeGrades, String gc_main_id, String validity) {
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user.setStore_apply_step(CommUtil.null2Int(step));
		this.userService.update(user);
		Store obj = user.getStore();
		...
		store.setGrade(this.storegradeService.getObjById(CommUtil.null2Long(storeGrades)));
		...
		for(GoodsClass gcp : goodsClass_parent){
			Map	gc_map = new HashMap();
			gc_map.put("m_id", gcp.getId());
			Set<GoodsClass> gcd = this.goodsClassService.getObjById(gcp.getId()).getChilds();
			List gc_list_gc = new ArrayList();
			for(GoodsClass gcc : gcd){
				Long gc_id = gcc.getId();
				gc_list_gc.add(gc_id);
			}
	 ...
	 * @return
	 */
	private ModelAndView seller_apply_step6(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("seller_apply/seller_apply_step6.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		Store user_store = user.getStore();
		if (user_store.getGrade().getMain_limit() == 1) {// 查出该主营类目下的二级类目
			GoodsClass gc = this.goodsClassService.getObjById(user_store.getGc_main_id());
			mv.addObject("gc", gc);
		} else {// 不限制
			List<GoodsClass> goodsClass = this.goodsClassService
					.query("select obj from GoodsClass obj where obj.parent.id is null", null, -1, -1);
		...
		Set<GoodsClass> all_details_gcs = this.storeTools.query_store_DetailGc(user_store.getGc_detail_info());
	...
	public String seller_apply_step6_save(HttpServletRequest request, HttpServletResponse response,
			String goodsClass_main, String step) {
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user.setStore_apply_step(CommUtil.null2Int(step));
		this.userService.update(user);
		Store store = user.getStore();
	 ...
	 * @return
	 */
	private ModelAndView seller_apply_step7(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("seller_apply/seller_apply_step7.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		mv.addObject("doc", this.documentService.getObjByProperty(null, "mark", "apply_agreement"));
		return mv;
	}
	...
	private ModelAndView seller_apply_money(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("seller_apply/seller_apply_money.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		List<GoodsClass> goodsClass = this.goodsClassService
				.query("select obj from GoodsClass obj where obj.parent.id is null", null, -1, -1);
		mv.addObject("goodsClass", goodsClass);
	...
	public ModelAndView seller_store_wait(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("seller_apply/seller_apply_wait.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
	...
	public ModelAndView seller_store_query(HttpServletRequest request, HttpServletResponse response) {
		...
		if (store.getStore_status() == 5 || store.getStore_status() == 6) {
			mv = new JModelAndView("seller_apply/seller_apply_wait1.html", configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 1, request, response);
		}
		if (store.getStore_status() == 10 || store.getStore_status() == 11) {
			mv = new JModelAndView("seller_apply/seller_apply_wait2.html", configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 1, request, response);
		}
		if (store.getStore_status() == 15 || store.getStore_status() == 20 || store.getStore_status() == 25) {
			mv = new JModelAndView("seller_apply/seller_apply_wait3.html", configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 1, request, response);
		}
		mv.addObject("store", store);
		return mv;
	...
	public ModelAndView seller_store_rewrite(HttpServletRequest request, HttpServletResponse response, String step) {
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
	...
	public void seller_apply_image_save(HttpServletRequest request, HttpServletResponse response, String mark,
			String uid, String jsessionid) {
		// HttpSessionContext SessCon=
		// request.getSession(false).getSessionContext();
		...
		Store store = user.getStore();
		// 上传
		String uploadFilePath = this.configService.getSysConfig().getUploadFilePath() + "/" + "store"+ "/" + store.getId();
		...
		if ("idCard".equals(mark)) {// 保存身份证电子版正面
			...
			try {
				map = CommUtil.httpsaveFileToServer(request, mark + "_img_a", uploadFilePath, "", null);
				if (map.get("fileName") != "") {
		...
		if ("idCard1".equals(mark)) {// 保存身份证电子版反面
			if (store.getLicense_legal_idCard_photo() != null) {// 如果存在，将其删除
				Long aid = store.getLicense_legal_idCard_photo().getId();
				store.setLicense_legal_idCard_photo(null);
				this.storeService.update(store);
				Accessory img = this.accessoryService.getObjById(CommUtil.null2Long(aid));
				boolean ret = this.accessoryService.delete(img.getId());
				if (ret) {
			...
			try {
				...
				if (map.get("fileName") != "") {
					...
					photo.setAddTime(new Date());
					this.accessoryService.save(photo);
					store.setLicense_legal_idCard_photo(photo);
					this.storeService.update(store);
					json.put("url", photo.getPath() + "/" + photo.getName());
					json.put("id", photo.getId());
				}
	...
	public void seller_apply_image_del(HttpServletRequest request, HttpServletResponse response, String mark,
			String uid) {
		...
		if ("idCard1".equals(mark)) {// 删除身份证电子版
			aid = store.getLicense_legal_idCard_photo().getId();
			store.setLicense_legal_idCard_photo(null);
			this.storeService.update(store);
			Accessory img = this.accessoryService.getObjById(CommUtil.null2Long(aid));
			try {
	...
	public ModelAndView seller_license_area_ajax(HttpServletRequest request, HttpServletResponse response, String value,
			String type) {
		ModelAndView mv = new JModelAndView("seller_apply/seller_store_ajax_select.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
	...
	public ModelAndView seller_goodsclass_ajax(HttpServletRequest request, HttpServletResponse response, String cid,
			String uid) {
		ModelAndView mv = new JModelAndView("seller_apply/seller_store_ajax_goodsClass.html",
		...
		while (it.hasNext()) {// 遍历集合
			...
			for (GoodsClass ugc : this.storeTools.query_store_DetailGc(user_store.getGc_detail_info())) {
	...
	public ModelAndView seller_goodsclass_save(HttpServletRequest request, HttpServletResponse response, String gc_ids,
			String uid) {
		ModelAndView mv = new JModelAndView("seller_apply/seller_store_gc_list.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		User user = this.userService.getObjById(CommUtil.null2Long(uid));
		Store store = user.getStore();
		...
		for (int i = 1; i < str.length; i++) {
			GoodsClass gc = this.goodsClassService.getObjById(CommUtil.null2Long(str[i]));
			parent = gc.getParent();
		}
		Map map_temp = this.storeTools.query_MainGc_Map(CommUtil.null2String(parent.getId()),
				store.getGc_detail_info());
		if (map_temp != null) {// 同一个一级类目
			gc_list = (List) map_temp.get("gc_list");
			List<GoodsClass> gc_mains = this.storeTools.query_store_detail_MainGc(store.getGc_detail_info());
		...
		for (int i = 1; i < str.length; i++) {
			GoodsClass gc = this.goodsClassService.getObjById(CommUtil.null2Long(str[i]));
			Set<GoodsClass> detail_info = this.storeTools.query_store_DetailGc(store.getGc_detail_info());
...
/*		
		...
		store.setGc_detail_info(Json.toJson(array_list, JsonFormat.compact()));*/
		map.put("m_id", parent.getId());
		map.put("gc_list", gc_list);
		list.add(map);
		store.setGc_detail_info(Json.toJson(list, JsonFormat.compact()));
		this.storeService.update(store);
		Set<GoodsClass> all_details_gcs = this.storeTools.query_store_DetailGc(store.getGc_detail_info());
		mv.addObject("details_gcs", all_details_gcs);
		return mv;
	}
	...
	public ModelAndView seller_goodsclass_del(HttpServletRequest request, HttpServletResponse response, String gc_id) {
		ModelAndView mv = new JModelAndView("seller_apply/seller_store_gc_list.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		Store store = user.getStore();
		GoodsClass gc_detail = this.goodsClassService.getObjById(CommUtil.null2Long(gc_id));
		GoodsClass gc_main = this.goodsClassService.getObjById(gc_detail.getParent().getId());
		Map map_temp = this.storeTools.query_MainGc_Map(CommUtil.null2String(gc_main.getId()),
				store.getGc_detail_info());
		...
		for (GoodsClass gc : gc_mains) {
			if (gc.getId() == gc_main.getId()) {// 删除对应类目
				...
				Map map_main = this.storeTools.query_MainGc_Map(CommUtil.null2String(gc.getId()),
						store.getGc_detail_info());
				list.add(map_main);
			}
		}
		store.setGc_detail_info(Json.toJson(list, JsonFormat.compact()));
		this.storeService.update(store);
		mv.addObject("details_gcs", this.storeTools.query_store_DetailGc(store.getGc_detail_info()));
		mv.addObject("store", store);
		return mv;
	}
