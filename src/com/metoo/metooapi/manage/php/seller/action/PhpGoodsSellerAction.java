...
public class PhpGoodsSellerAction {
	...
	public void add_goods_second(HttpServletRequest request,
			HttpServletResponse response, String user_id, String gc_id) {
			Result result = null;
			...
			if (store_status == 15) {
				...
				if (gc.getLevel() == 2) {// 发布商品选择分类时选择三级分类,查询出所有与该三级分类关联的规格，即规格对应的详细商品分类
					...
					List<GoodsSpecification> goods_spec_list = this.goodsSpecificationService.query("select obj from GoodsSpecification obj where obj.spec_type=:spec_type order by obj.sequence asc",
							spec_map, -1, -1);
				...
				goods_info.put("imageSuffix",  this.storeViewTools
	...
	public void php_add_goods_finish(HttpServletRequest request,
			...
			String delivery_area_id, String goods_price, String store_price, String user_id, String goods_transfee) {
			...
			if (m) {
				...
				if(!maps.isEmpty()){
					...
					goods.setGoods_inventory(CommUtil.null2Int(goods_inventory_list) == 0 ? 0 : CommUtil.null2Int(goods_inventory_list));
				}
	...
	public void _cgoods(Long goodsId, List<Map> specList){
		...
		for(Map spec : specList){
			...
			BigDecimal cgoods_current_price = CommUtil.null2BigDecimal(spec.get("supp").equals("") ? 0 : spec.get("supp"));
	...
	/*@RequestMapping("/php/seller/goods_edit.htm")
	public void goods_edit(HttpServletRequest request,
		...
		if (obj.getGoods_store().getUser().getId().equals(user.getId())) {
			...
			goods_info.put("ugcs", ugc_list);
			
			List<GoodsSpecProperty> specs = obj.getGoods_specs();
			List<Map> spec_lists = new ArrayList<Map>();
			for(GoodsSpecProperty spec : specs){
				Map spec_map = new HashMap(); 
				spec_map.put("spec_id", spec.getId());
				spec_map.put("spec_value", spec.getValue());
				spec_lists.add(spec_map);
			}
			goods_info.put("goods_properties", spec_lists);
				if (obj.getGc() != null) {
					Map spec_map = new HashMap();
					spec_map.put("store_id", user.getStore().getId());
					List<GoodsSpecification> goods_spec_list = this.goodsSpecificationService
				...
				if(obj.getInventory_type().equals("all")){
					goods_info.put("goods_id", obj.getId());
					goods_info.put("goods_name", obj.getGoods_name());
					goods_info.put("goods_price", obj.getGoods_price());
					goods_info.put("goods_store_price", obj.getStore_price());
					goods_info.put("goods_inventory", obj.getGoods_inventory());
					goods_info.put("goods_inventory_type", obj.getInventory_type());
					goods_info.put("goods_inventory_detail", obj.getGoods_inventory_detail());
					goods_info.put("goods_weight", obj.getGoods_weight());
					goods_info.put("goods_length", obj.getGoods_length());
					goods_info.put("goods_width", obj.getGoods_width());
					goods_info.put("goods_high", obj.getGoods_high());
					goods_info.put("goods_transfee", obj.getGoods_transfee());
					goods_info.put("goods_features_one", obj.getFeatures_one());
					goods_info.put("goods_features_two", obj.getFeatures_two());
					goods_info.put("goods_features_three", obj.getFeatures_three());
					goods_info.put("goods_features_four", obj.getFeatures_four());
					goods_info.put("goods_features_five", obj.getFeatures_five());
					goods_info.put("publish_goods_status", obj.getPublish_goods_status());
					goods_info.put("goods_choice_type", obj.getGoods_choice_type());
					goods_info.put("goods_cod", obj.getGoods_cod());
					
					goods_info.put("goods_main_photo", this.configService.getSysConfig().getImageWebServer() +"/"+ obj.getGoods_main_photo().getPath() +"/"+ obj.getGoods_main_photo().getName());
					Iterator<Accessory> photos = obj.getGoods_photos().iterator();
					List<Map> accessorys = new ArrayList<Map>();
					while(photos.hasNext()){
						Accessory photo = photos.next();
						Map photo_map = new HashMap();
						photo_map.put("photo_id", photo.getId());
						photo_map.put("photo_img", this.configService.getSysConfig().getImageWebServer() +"/"+ photo.getPath() +"/"+ photo.getName());
						accessorys.add(photo_map);
					}
					goods_info.put("accessorys", accessorys);
					
				}else{
					List<Map> cgoods_list = new ArrayList<Map>();
					Iterator<CGoods> cgoodses = obj.getCgoods().iterator();
					while(cgoodses.hasNext()){
						CGoods cg = cgoodses.next();
						Map cg_map = new HashMap();
						cg_map.put("goods_id", cg.getId());
						cg_map.put("goods_serial", cg.getGoods_serial());
						cg_map.put("eid", cg.getEid());
						cg_map.put("goods_combination_id", cg.getCombination_id());
						cg_map.put("goods_disabled", cg.getGoods_disabled());
						cg_map.put("goods_name", cg.getGoods().getGoods_name());
						cg_map.put("goods_price", cg.getGoods_price());
						cg_map.put("goods_discount_price", cg.getDiscount_price());
						cg_map.put("goods_inventory", cg.getGoods_inventory());
						cg_map.put("goods_width", cg.getGoods_width());
						cg_map.put("goods_length", cg.getGoods_length());
						cg_map.put("goods_high", cg.getGoods_high());
						cg_map.put("goods_weight", cg.getGoods_weight());
						cg_map.put("goods_transfee", cg.getGoods().getGoods_transfee());
						cg_map.put("goods_inventory_type", cg.getGoods().getInventory_type());
						cg_map.put("goods_features_one", cg.getGoods().getFeatures_one());
						cg_map.put("goods_features_two", cg.getGoods().getFeatures_two());
						cg_map.put("goods_features_three", cg.getGoods().getFeatures_three());
						cg_map.put("goods_features_four", cg.getGoods().getFeatures_four());
						cg_map.put("goods_features_five", cg.getGoods().getFeatures_five());
						cg_map.put("publish_goods_status", cg.getGoods().getPublish_goods_status());
						cg_map.put("goods_choice_type", cg.getGoods().getGoods_choice_type());
						cg_map.put("goods_cod", cg.getGoods().getGoods_cod());
						List<Map> photos = new ArrayList<Map>();
						Iterator<Accessory> accessorys = cg.getC_goods_photos().iterator();
						while(accessorys.hasNext()){
							Accessory photo = accessorys.next();
							Map photo_map = new HashMap();
							photo_map.put("photo_id", photo.getId());
							photo_map.put("photo_img", this.configService.getSysConfig().getImageWebServer() +"/"+ photo.getPath() +"/"+ photo.getName());
							photos.add(photo_map);
						}
						cg_map.put("photos", photos);
						cgoods_list.add(cg_map);
					}
					goods_info.put("goods_list", cgoods_list);
				}
		
			goods_info.put("goods_store_id", obj.getGoods_store().getId());
			goods_info.put("goods_user_id", obj.getGoods_store().getUser().getId());
			result = new Result(0,"success",goods_info);
		} else {
			result = new Result(0,"没有该商品");
		}
		try {
			response.getWriter().print(Json.toJson(result, JsonFormat.compact()));
		} catch (IOException e) {
	...
	public void goods_image_del(HttpServletRequest request,
			HttpServletResponse response, String image_id, String user_id) {
		User user = null;
		Result result = null;
	...
	public void goodsSale(HttpServletRequest request,
			HttpServletResponse response, String mulitId, String uid){
		Result result = null;
		int ret = -1;
	...
	public void phpGoodsDelete(HttpServletRequest request,
			HttpServletResponse response,String mulitId, String uid){
		Result result = null;
		String[] ids = mulitId.split(",");
