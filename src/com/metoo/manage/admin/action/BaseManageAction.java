...
public class BaseManageAction {
		...
		if (SecurityUserHolder.getCurrentUser() != null) {
			...
			if (ajax_login) {
				...
				//如果登陆成功，颁发token给客户端
					...
					User userinfo =  this.userService.getObjById(SecurityUserHolder
							.getCurrentUser().getId());
					usermap.put("truename", userinfo.getTrueName());
					...
					if(userinfo.getSex() == -1){
						usermap.put("user_photo", this.configService.getSysConfig().getImageWebServer()+"/"+"resources"+"/"+"style"+"/"+"common"+"/"+"images"+"/"+"member.png");
					}
					if(userinfo.getSex() == 0){
						usermap.put("user_photo", this.configService.getSysConfig().getImageWebServer()+"/"+"resources"+"/"+"style"+"/"+"common"+"/"+"images"+"/"+"member0.png");
					}
					if(userinfo.getSex() == 1){
						usermap.put("user_photo", this.configService.getSysConfig().getImageWebServer()+"/"+"resources"+"/"+"style"+"/"+"common"+"/"+"images"+"/"+"member1.png");
					}
					usermap.put("sex", userinfo.getSex());
					usermap.put("id", userinfo.getId());
				
					Result result = new Result(0,"请跳转登陆页面",usermap);
					url = Json.toJson(result,JsonFormat.compact());
	...
	public void logout_success(HttpServletRequest request,
		...
		/*String targetUrl = CommUtil.getURL(request) + "/user_login.json";*/
	...
	public ModelAndView login_error(HttpServletRequest request,
		...
		String metoo_view_type = CommUtil.null2String(request.getSession(
				false).getAttribute("metoo_view_type"));
	...
	public ModelAndView welcome(HttpServletRequest request,
		...
		List<StoreStat> states = this.storeStatService.query(
	...
	public ModelAndView sys_config_save(HttpServletRequest request,
		...
		try {
			...
			map = CommUtil.httpsaveFileToServer(request, "websiteLogo",
		...
		map.clear();
		try {
			map = CommUtil.httpsaveFileToServer(request, "goodsImage",
		...
		map.clear();
		try {
			map = CommUtil.httpsaveFileToServer(request, "storeImage",
					saveFilePathName, null, null);
		...
		map.clear();
		try {
			map = CommUtil.httpsaveFileToServer(request, "memberIcon",
					saveFilePathName, null, null);
		...
		map.clear();
		try {
			map = CommUtil.httpsaveFileToServer(request, "admin_login_img",
					saveFilePathName, null, null);
			String fileName = this.configService.getSysConfig()
		...
		map.clear();
		try {
			map = CommUtil.httpsaveFileToServer(request, "admin_manage_img",
					saveFilePathName, null, null);
			String fileName = this.configService.getSysConfig()
		...
		map.clear();
		try {
			map = CommUtil.httpsaveFileToServer(request, "qrLogo",
					saveFilePathName, null, null);
		...
		for (int i = 0; i < 4; i++) {
			try {
				...
				map = CommUtil.httpsaveFileToServer(request, "img" + i,
						saveFilePathName, fileName, null);
				if (fileName.equals("")) {
					if (map.get("fileName") != "") {
	...
	public void upload(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			Map map = CommUtil.httpsaveFileToServer(request, "imgFile",
					saveFilePathName, null, null);
			/*String url = webPath + "/"
					+ this.configService.getSysConfig().getUploadFilePath()
					+ "/common/" + map.get("fileName");*/
			String url = this.configService.getSysConfig().getImageWebServer() +"/"+ saveFilePathName + "/" + map.get("fileName");
			obj.put("error", 0);
			obj.put("url", url);
		} catch (IOException e) {
			obj.put("error", 1);
	...
	public void email(HttpServletResponse response, String email, String content) {
		String content1 = "";
		switch (content) {
		case "5": content1 = "正在审核公司信息";
			break;
		case "6": content1 = "公司审核拒绝";
			break;
		case "10": content1="正在进行店铺审核";
			break;
		case "11": content1="店铺审核拒绝";
			break;
		case "15": content1="正常营业";
			break;
		case "20": content1="违规关闭";
			break;
		case "25": content1="到期关闭";
			break;
		case "26": content1="到期已申请续费";
			break;
		}
