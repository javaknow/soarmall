...
public class StoreSellerAction {
	...
	@SecurityMapping(title = "店铺满额包邮", value = "/seller/store_set.htm*", rtype = "seller", rname = "店铺设置", rcode = "store_set_seller", rgroup = "我的店铺")
	@RequestMapping("/seller/fulfil_set.htm")
	public ModelAndView fulfil_set(HttpServletRequest request,
			HttpServletResponse response) {
		ModelAndView mv = new JModelAndView(
				"user/default/sellercenter/fulfil.html",
				configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = this.userService.getObjById(SecurityUserHolder
				.getCurrentUser().getId());
	...
	@SecurityMapping(title = "店铺满额包邮", value = "/seller/store_set.htm*", rtype = "seller", rname = "店铺设置", rcode = "store_set_seller", rgroup = "我的店铺")
	@RequestMapping("/seller/fulfil_save.htm")
	public void fulfil_save(HttpServletRequest request,
			HttpServletResponse response, BigDecimal price) {
		ModelAndView mv = new JModelAndView(
				"user/default/sellercenter/fulfil.html",
				configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
	...
	public void store_set_save(HttpServletRequest request,
		...
		String saveFilePathName = this.configService.getSysConfig()
				.getUploadFilePath() +"/store_logo";
		Map map = new HashMap();
		try {
			String fileName = store.getStore_logo() == null ? "" : store
					.getStore_logo().getName();
			map = CommUtil.httpsaveFileToServer(request, "logo", saveFilePathName,
					fileName, null);
			if (fileName.equals("")) {
				if (map.get("fileName") != "") {
		...
		/*saveFilePathName = request.getSession().getServletContext()
				...
				+ "/store_banner";*/
		 saveFilePathName = this.configService.getSysConfig()
				.getUploadFilePath() +"/store_banner";
		
		try {
			String fileName = store.getStore_banner() == null ? "" : store
					.getStore_banner().getName();
			map = CommUtil.httpsaveFileToServer(request, "banner",
