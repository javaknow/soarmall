...
public class StoreAdjustApplyAction {
	...
	public ModelAndView adjust_apply_save(HttpServletRequest request,
		...
		if(infos.size()==0){
			...
			String grade_id = store_grade.substring(0, store_grade.length()-1);
			StoreGrade sg = this.storeGradeService.getObjById(CommUtil.null2Long(grade_id));
			storeAdjustInfo.setAdjust_store_grade(sg.getGradeName());
			storeAdjustInfo.setAdjust_storeGrade_id(CommUtil.null2Long(grade_id));
