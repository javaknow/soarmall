	...
	public void upload_evaluate(HttpServletRequest request,
		...
		if (imglist.size() > 40) {// 每个用户最多可以上传40张临时图片，半小时清理一次
			...
			try {
				...
				Map map = CommUtil.httpsaveFileToServer(request,
		...
		if (evaluate_session1 != null
			...
			if (obj != null
				...
				if (obj.getOrder_status() == 40) {
					...
					for (Map map : json) {
						...
						if (description == 0 || service == 0 || ship == 0
							...
							ModelAndView mv = new JModelAndView("error.html",
									...
									request, response);
							mv.addObject("op_title", "Error, no evaluation allowed");
							mv.addObject("url", CommUtil.getURL(request)
									+ "/buyer/order.htm");
							return mv;
						}
					...
					OrderFormLog ofl = new OrderFormLog();
					ofl.setAddTime(new Date());
					ofl.setLog_info("Comment the order");
					ofl.setLog_user(SecurityUserHolder.getCurrentUser());
					ofl.setOf(obj);
					this.orderFormLogService.save(ofl);
					...
					for (Map map : json) {
						...
						if (this.configService.getSysConfig().isIntegral()) {
							...
							log.setContent("Order evaluation increases"
									+ this.configService.getSysConfig()
											.getIndentComment() + "score");
			...
			ModelAndView mv = new JModelAndView("success.html",
					...
					this.userConfigService.getUserConfig(), 1, request,
					response);
			mv.addObject("op_title", "Order evaluation successful");
			mv.addObject("url", CommUtil.getURL(request) + "/buyer/order.htm");
			...
			for (Goods goods : goods_list) {
				...
				System.out.println(goods.getWell_evaluate());
			...
			ModelAndView mv = new JModelAndView("error.html",
					...
					this.userConfigService.getUserConfig(), 1, request,
					response);
			mv.addObject("op_title", "No repeat evaluation!");
			mv.addObject("url", CommUtil.getURL(request) + "/buyer/order.htm");
			return mv;
		}
