...
public class CommUtil {
	 ...
	 */
	public static Map saveAccountFileToServer(HttpServletRequest request,
			MultipartFile file, String saveFilePathName, String saveFileName,
	...
	public static String showPageFormHtml(int currentPage, int pages) {
		...
		if (pages > 0) {
			if (currentPage >= 1) {
				s += "<a class='btn_first' href='javascript:void(0);' title='首页' onclick='return gotoPage(1)'>First</a> ";
				if (currentPage > 1)
	...
	public static List<Map> saveIPageList2ModelAndView2(IPageList pList){
		...
		for (Address addresslist : address) {
			...
			addressMap.put("consigneeName", addresslist.getTrueName());
			...
			addressMap.put("default_val", addresslist.getDefault_val());
			try {
				addressMap.put("areaChildName", addresslist.getArea()
						.getAreaName());
				addressMap.put("areaName",addresslist.getArea()
						.getParent()
						.getAreaName());
			} catch (Exception e) {
				addressMap.put("areaChildName", "");
				addressMap.put("areaName","");
			}
		
			addressMap.put("areaUserName",addresslist.getUser()
					.getTrueName());
			if(addresslist.getArea().getParent() !=null){
				addressMap.put("areaParendName",addresslist.getArea()
						.getParent()
						.getParent()
						.getAreaName());
			}else{
    ...
    /**
     *  已被修改代码-修改前图片上传至项目文件夹
	 *  httpsaveFileToServer 上传文件保存到服务器
	 * 
	 * @param filePath为上传文件的
	 * @param saveFilePathName为文件保存全路径
	 * @param saveFileName为保存的文件
	 * @param extendes为允许的文件扩展名
	 *            , *
	 ...
	 *         第三个为保存文件时错误信息errors,如果生成缩略图则map中保存smallFileName，表示缩略图的全路径
	 */
    public static Map httpsaveFileToServer(HttpServletRequest request,
		...
		CommonsMultipartFile commonsMmultipartFile = (CommonsMultipartFile) multipartRequest
				.getFile(filePath);
	 if (commonsMmultipartFile != null && !commonsMmultipartFile.isEmpty()) {
		/*MultipartFile multipartFile = commonsMmultipartFile; 
        CommonsMultipartFile cf= (CommonsMultipartFile)multipartFile; */
        DiskFileItem diskFileItem = (DiskFileItem)commonsMmultipartFile.getFileItem(); 
        File file = diskFileItem.getStoreLocation();
       
			System.out.println("文件名为：" + commonsMmultipartFile.getOriginalFilename());
			String extend = commonsMmultipartFile.getOriginalFilename()
					.substring(commonsMmultipartFile.getOriginalFilename().lastIndexOf(".") + 1)
					.toLowerCase();
			if (saveFileName == null || saveFileName.trim().equals("")) {
				saveFileName = UUID.randomUUID().toString() + "." + extend;
			}
			...
			float fileSize = Float.valueOf(commonsMmultipartFile.getSize());// 返回文件大小，单位为k
			List<String> errors = new java.util.ArrayList<String>();
			boolean flag = true;
			if (extendes == null) {
				extendes = new String[] { "jpg", "jpeg", "gif", "bmp", "tbi",
			...
			if (flag) {
		          ...
		          try {
		            ...
		            */
		            ...
		            MultipartEntityBuilder entityBuilder = MultipartEntityBuilder.create();//向一个Http实体添加成分 是向一个HttpEntity添加成分来表示表单的一般方法.
			        entityBuilder.addPart("file", new FileBody(file));
		         	entityBuilder.addTextBody("path", httpfilePath);//添加普通参数
		            ...
		            HttpEntity  resEntity  = response.getEntity();
		             if (resEntity != null) {
		                 System.out.println("Response content length: " + resEntity.getContentLength());// 打印响应长度
		                 String result=EntityUtils.toString(response.getEntity());
		                 JSONObject json = JSONObject.parseObject(result);
		                 code = json.get("code").toString();
		                 if(code != null && code.equals("0")){
			                 String jsonConversionString = json.get("msg").toString();
		               ...
		               /*  JsonParser parse =new JsonParser();  //创建json解析器
		                 JsonObject json=(JsonObject) parse.parse(EntityUtils.toString(resEntity, Charset.forName("UTF-8"))); //创建jsonObject对象
		                 code  = json.get("code").getAsInt();
		                 if(code == 0){
			                 String jsonConversionString = json.get("msg").getAsString();
			                 JsonObject data = json.get("data").getAsJsonObject();
			                 String goodsName = data.get("img").getAsString();
			                 imgName = goodsName.substring(goodsName.lastIndexOf("/")+1);
			                 goodsExtend = imgName.substring(imgName.lastIndexOf(".")+1).toLowerCase();
		                 }*/
		             } 
